...
inline BigInteger __pow__(BigInteger x, int n)
{
    BigInteger r = 1;
...
int main()
{
    ...
    while (~scanf("%d%d", &n, &c))
    {
        nn = n * n;
        res = 0;
        res += __pow__(c, (nn >> 2) + (n & 1)) * 2 + __pow__(c, (nn >> 1) + (n & 1)) + __pow__(c, nn);
        if (n & 1) res += __pow__(c, n * (n + 1 >> 1)) * 4;
        else res += (__pow__(c, nn >> 1) + __pow__(c, (n + 1) * (n >> 1))) * 2;
        res /= 8;
        res.println();
    }
    return 0;
